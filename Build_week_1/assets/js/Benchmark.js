...
function showQuestion() {
  ...
  questionNumber.innerHTML = `QUESTION ${currentQuestionIndex + 1} <span>/${
    questions.length
  }</span>`;
...
let timerId; // variabile globale per salvare l'ID del timer

// Funzione che mostra il risultato del quiz
function showScore() {
  clearInterval(timerId); // interrompi il timer se è ancora attivo
  const timerDiv = document.getElementById("timer"); // ottieni il riferimento al div del timer
  timerDiv.style.display = "none"; // nascondi il div del timer
